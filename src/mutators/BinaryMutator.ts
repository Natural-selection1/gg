        ...
        if (from.type == "Revision") {
            ...
            if (from.hunk) {
                return {
                    type: "yes",
                    hint: [
                        `Squashing hunk ${from.hunk.location.from_file.start}:${from.hunk.location.from_file.start + from.hunk.location.from_file.len}@${from.path.relative_path} from revision `,
                        from.header.id.change,
                    ],
                };
            } else {
                return {
                    type: "yes",
                    hint: [
                        `Squashing file ${from.path.relative_path} from revision `,
                        from.header.id.change,
                    ],
                };
            }
            ...
            if (this.#to.type == "Revision") {
                if (this.#to.header.id.change.hex == this.#from.header.id.change.hex) {
                    ...
                    return {
                        type: "yes",
                        hint: [
                            `Squashing changes from ${this.#from.path.relative_path} into `,
                            this.#to.header.id.change,
                        ],
                    };
                }
